<div class="p-2">
    <div class="max-w-7xl mx-auto px-2 lg:px-8">
        <div class="flex gap-2">
            <!-- Income Form -->
            <div class="w-1/3 bg-white dark:bg-zinc-800 shadow-sm rounded-lg p-6">
            ...
            <div class="w-2/3 flex flex-col gap-2">
                ...
                <div class="bg-white dark:bg-zinc-800 shadow-sm rounded-lg p-6">
                    <div class="flex justify-between items-center">
                        <x-input-label class="text-zinc-700 dark:text-zinc-300">Total Incomes</x-input-label>
                        <h3 class="text-xl font-bold text-zinc-700 dark:text-zinc-300">RM {{ number_format($totalIncomes, 2) }}</h3>
                    </div>
                </div>
                    ...
                    <table class="table min-w-full">
                        <thead>
                            <tr class="text-bold text-zinc-700 dark:text-zinc-300">
                                ...
                                <th class="py-2 px-4 border-b border-customBlue">Source</th>
                        ...
                        <tbody>
                            @foreach ($incomes as $income)
                                <tr class="text-center">
                                    <td class="py-2 px-4 w-48 text-center text-zinc-700 dark:text-zinc-300 border-b border-zinc-700">{{ $income->description }}</td>
                                    <td class="py-2 px-4 text-center text-zinc-700 dark:text-zinc-300 border-b border-zinc-700">RM {{ number_format($income->amount, 2) }}</td>
                                    <td class="py-2 px-4 text-center text-zinc-700 dark:text-zinc-300 border-b border-zinc-700">{{ $income->source->name }}</td>
                                    <td class="py-2 px-4 text-center text-zinc-700 dark:text-zinc-300 border-b border-zinc-700">{{ $income->date }}</td>
                                    <td class="py-2 px-4 text-center border-b border-zinc-700">
                                        <x-primary-button wire:click="edit({{ $income->id }})">Edit</x-primary-button>
                                        <x-secondary-button wire:click="delete({{ $income->id }})" class="ml-2">Delete</x-secondary-button>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
